...
/**
 * REST controller for managing {@link com.leafpay.domain.Log}.
 */
@RestController
...
public class LogResource {
    ...
    public ResponseEntity<LogDTO> createLog(@Valid @RequestBody LogDTO logDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/logs/" + logDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, logDTO.getId().toString()))
            .body(logDTO);
    }
    ...
    public ResponseEntity<LogDTO> updateLog(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody LogDTO logDTO)
        ...
        logDTO = logService.update(logDTO);
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<LogDTO>> getAllLogs(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<LogDTO> getLog(@PathVariable("id") Long id) {
